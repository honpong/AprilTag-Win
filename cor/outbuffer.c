...
packet_t *outbuffer_write(struct outbuffer *mb, uint64_t *offset)
{
...
void *outbuffer_start(struct outbuffer *mb)
{
    uint64_t thread_pos = 0;
    while (outbuffer_write(mb, &thread_pos)) {
        pthread_testcancel();
    }
    return NULL;
}
...
struct plugin outbuffer_open(struct outbuffer *mb)
{
    ...
    return (struct plugin) {.data=mb, .start=(void *(*)(void *))outbuffer_start, .stop=(void (*)(void *))outbuffer_close};
}

void outbuffer_copy_packet(struct outbuffer *mb, packet_t *p) {
    packet_t *p2 = outbuffer_alloc(mb, p->header.type, p->header.bytes-16);
    memcpy(p2->data, p->data, p->header.bytes-16);
    p2->header.user = p->header.user;
    outbuffer_enqueue(mb, p2, p->header.time);
}
