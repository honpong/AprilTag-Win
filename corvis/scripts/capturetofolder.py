...
if len(sys.argv) != 3:
    print "Usage:", sys.argv[0], "<capture_filename> <output_folder>"
    sys.exit(1)
...
while p is not None:
    ...
    if p.header.type == PacketType.accelerometer or p.header.type == PacketType.gyroscope:
        ...
        if p.header.type == PacketType.accelerometer: accel_file.write(line)
        if p.header.type == PacketType.gyroscope: gyro_file.write(line)
        
    if p.header.type == PacketType.image_raw:
