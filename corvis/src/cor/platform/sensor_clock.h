...
public:
    ...
    typedef std::chrono::duration<uint64_t, std::ratio<1, 10000000>> ns100;
    ...
    static uint64_t tp_to_ns100(const time_point t) { return std::chrono::duration_cast<ns100>(t.time_since_epoch()).count(); }
    static time_point ns100_to_tp(const uint64_t m) { return time_point(std::chrono::duration_cast<duration>(ns100(m))); }
};
