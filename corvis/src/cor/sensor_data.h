...
public:
    ...
    image_data(const image_data<camera_type, data_type> &other, bool init = true) : timestamp(other.timestamp), exposure_time(other.exposure_time),
    ...
    image_data(image_data<camera_type, data_type>&& other, data_type initial_value) : image_data(other, false) {
        for (int y=0; y<height; y++) std::fill_n(image, width, initial_value);
    }
