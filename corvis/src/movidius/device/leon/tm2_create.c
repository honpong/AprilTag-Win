...
static VOID UsbPumpVsc2App_ClientDelete();

static USBPUMP_API_OPEN_CB_FN UsbPumpVsc2App_OpenSession_Callback;
...
USBPUMP_VSC2APP_CONTEXT * UsbPumpVsc2App_ClientCreate(UPLATFORM * pPlatform)
{
    ...
    if(pSelf == NULL) {
        TTUSB_PLATFORM_PRINTF((pPlatform, UDMASK_ERRORS | UDMASK_ANY,
                               "?UsbPumpVsc2App_ClientCreate:"
	...
	*/
    ...
    pObjectHeader = UsbPumpObject_EnumerateMatchingNames(
            &pRootObject->Header, NULL, gk_ProtoVsc2_ObjectName);
    ...
    if(StatusOk) {
        CONST RECSIZE SizeOpenMemory =
                UsbPumpObject_SizeOpenSessionRequestMemory(pObjectHeader);

        UsbPumpObject_OpenSession(
                pObjectHeader,
                UsbPumpPlatform_Malloc(pPlatform, SizeOpenMemory),
                SizeOpenMemory, UsbPumpVsc2App_OpenSession_Callback,
                pSelf,                           /* pCallBackContext */
                &gk_UsbPumpProtoVsc2_Guid, NULL, /* pClientObject -- OPTIONAL */
                &pSelf->InCall.GenericCast, sizeof(pSelf->InCall),
                pSelf, /* pClientHandle */
                &gk_UsbPumpProtoVsc2_OutCall.GenericCast,
                sizeof(gk_UsbPumpProtoVsc2_OutCall));

        TTUSB_PLATFORM_PRINTF((pPlatform, UDMASK_ANY,
                               " UsbPumpVsc2App_ClientCreate:"
                               " pSelf(%p) created for pProtoVscObject(%p).\n",
                               pSelf, pSelf->pProtoVscObject));
...
static VOID UsbPumpVsc2App_ClientDelete()
{
    ...
    while((pQe = UsbGetQe(&pSelf->pFreeQeHeadOut)) != NULL) {
        ...
        UsbFreeDeviceBuffer(pSelf->pDevice, pRequest->pBuffer,
                            pRequest->nBuffer);
    }
...
static VOID UsbPumpVsc2App_OpenSession_Callback(
        VOID * pClientContext, USBPUMP_SESSION_HANDLE SessionHandle,
        UINT32 Result, VOID * pOpenRequestMemory, RECSIZE sizeOpenRequestMemory)
{
    pSelf = pClientContext;
    USBPUMP_PROTO_VSC2_STATUS Status;
    UINT32                    i;

    TTUSB_PLATFORM_PRINTF((pSelf->pPlatform, UDMASK_ENTRY | UDMASK_ANY,
                           "+UsbPumpVsc2App_OpenSession_Callback:"
                           " Result=%d, SessionHandle(%#x)\n",
                           Result, SessionHandle));

    if(pOpenRequestMemory != NULL) {
        UsbPumpPlatform_Free(pSelf->pPlatform, pOpenRequestMemory,
    ...
    for(i = 0; i < USBPUMP_VSC2APP_NUM_EP_OUT; i++) {
        Status = (*pSelf->InCall.Vsc.pOpenStreamFn)(
                pSelf->hSession,
                UPIPE_SETTING_MASK_BULK_OUT | UPIPE_SETTING_MASK_INT_OUT,
                0, /* EpAddrMask */
                i, /* OrdPipe */
                &pSelf->hStreamOut[i]);
    ...
    for(i = 0; i < USBPUMP_VSC2APP_NUM_EP_IN; i++) {
        Status = (*pSelf->InCall.Vsc.pOpenStreamFn)(
                pSelf->hSession,
                UPIPE_SETTING_MASK_BULK_IN | UPIPE_SETTING_MASK_INT_IN,
                0, /* EpAddrMask */
                i, /* OrdPipe */
                &pSelf->hStreamIn[i]);
