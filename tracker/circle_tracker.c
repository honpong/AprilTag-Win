...
int findCircles(IplImage *img, CvSize size, CvPoint2D32f *features);
...
static void addfeatures(struct tracker *t, int newfeats)
{
    ...
    /*    size.width = 8;
    size.height = 6;
    int res = cvFindChessboardCorners(t->header1, size, (CvPoint2D32f*) newfeatures, &newfeats, CV_CALIB_CB_ADAPTIVE_THRESH);*/
    size.width = 4;
    size.height = 11;
    int res = findCircles(t->header1, size, (CvPoint2D32f*) newfeatures);
    newfeats = 44;
    ...
    fprintf(stderr, "findchess returned %d\n", res);
    if(!res) return;
...
void frame(void *_t, packet_t *p)
{
    ...
    //size.height = 6;
    int oldfeats = 44;
    //int res = cvFindChessboardCorners(t->header1, size, (CvPoint2D32f*) trackedfeats, &oldfeats, CV_CALIB_CB_ADAPTIVE_THRESH);
    
    size.width = 4;
    size.height = 11;
    int res = findCircles(t->header1, size, (CvPoint2D32f*) trackedfeats);

    fprintf(stderr, "findchess returned %d\n", res);
    if(!res) return;

    //    cvFindCornerSubPix(t->header1, (CvPoint2D32f *)trackedfeats, oldfeats, t->optical_flow_window, cvSize(-1,-1), t->optical_flow_termination_criteria);
